import { withCursor, type KvItem } from './helper';
...
> {
   ...
   * @param key 唯一key
   */
  async get(key: string): Promise<{ metadata: S; value: R } | null> {
    const { kv } = this;
   ...
   */
  async getSimple(key: string, cacheTtl?: number): Promise<R | null> {
    const { kv, type } = this;
   ...
   * @param value 值
   * @param metadata 元数据
   * @param option-expiration 使用以 UNIX 纪元以来的秒数指定的绝对时间设置密钥的过期时间. 例如，如果您希望密钥在 2019 年 4 月 1 日凌晨 12:00 UTC 过期，您可以将密钥的过期时间设置为1554076800。
   * @param option-expirationTtl 使用从当前时间算起的相对秒数设置密钥的有效期 (TTL)。例如，如果您希望密钥在创建后 10 分钟过期，您可以将其过期 TTL 设置为600。
   */
  async push(
    key: string,
    value: R,
    metadata: Partial<S>,
    option: { expiration?: number; expirationTtl?: number } = {},
  ): Promise<void> {
    const { kv } = this;
    const json = JSON.stringify(value);

    await kv.put(key, json, { ...option, metadata });
  }
