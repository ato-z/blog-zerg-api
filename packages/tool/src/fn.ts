import { isNumber, toNumber } from './checkTypes';

export { isNumber, toNumber } from './checkTypes';
...
/**
 * 字符串轉對象 a=1&b2 => { a:1, b:2 }
 */
export const partParams = <T>(param: string | undefined): T => {
  if (param === undefined) {
...
/**
 * 解析路由： https://blog-serverless.a-toz.workers.dev/a/b/c
 */
  ...
  lis
    ...
    .forEach((item: string) => {
      if (/Content-Disposition:\s+form-data;\s+name=/i.test(item)) {
        ...
        const name = reg![1] as unknown as string;
